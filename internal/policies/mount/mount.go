...
func (m *Manager) cleanup(ctx context.Context, objectName string, isComputer bool) (err error) {
	defer decorate.OnError(&err, gotext.Get("failed to clean up mount policy files for %q", objectName)) //nolint:govet // printf: this is an i18n formatted const string

	log.Debugf(ctx, gotext.Get("Cleaning up mount policy files for %q", objectName)) //nolint:govet // printf: this is an i18n formatted const string
